...
export async function interactWithGenius(
  ...
  fileBase64?: string,
  mimeType?: string
): Promise<{ text: string; html?: string; errorCode?: string }> {
  try {
    const ai = getAI();
...
ESTILO DE DESIGN: ${
      persona.style === "scientific"
        ? "Pergaminho, giz, diagramas técnicos e precisão."
        : persona.style === "modern"
        ? "Minimalismo, Tailwind CSS, funcionalidade extrema."
        : "Rico em texturas, cores vibrantes e narrativa visual."
    }
...
FORMATO DE RESPOSTA:
Sempre use the following formato se houver código:
[TEXTO DA PERSONA]
\`\`\`html
[CÓDIGO AQUI]
\`\`\`
Se não houver código, apenas o [TEXTO DA PERSONA].`;
    ...
    if (err?.message === "API_KEY_MISSING") {
      return { text: "", errorCode: "API_KEY_MISSING" };
    }
    return { text: "Falha ao chamar o Gemini. Verifique sua chave e tente novamente.", errorCode: "GEMINI_CALL_FAILED" };
  }
}
